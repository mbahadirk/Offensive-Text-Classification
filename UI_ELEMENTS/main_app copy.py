...
import threading
import asyncio # Bu import zaten vardı, kalsın
...
try:
    ...
    ModelManager = None
    ExtendedBinarySentimentClassifier=None
    BertClassifier = None
    BertLSTMClassifier = None

# Ön işleme modülünü import edin
try:
    # Hem clean_html_tags_and_time hem de full_cleaning_pipeline'ı import ediyoruz
    from preprocessing_module import clean_text, full_cleaning_pipeline # 'clean_html_tags_and_time' yerine 'clean_text' kullanıldı
    print("Preprocessing Module loaded.")
    # PREPROCESSING_AVAILABLE bayrağını burada True olarak ayarlıyoruz
    ...
    full_cleaning_pipeline = None # Eğer import hatası olursa bunu da None yapın
    PREPROCESSING_AVAILABLE = False # Metin ön işleme devre dışı
...
# Not: preprocess_derin_raw.py ve x3.py'nin aynı dizinde veya Python path'inizde olduğundan emin olun.


try:
    from googleapiclient.discovery import build # build fonksiyonu burada kalmalı
...
try:
    from x3 import TwitterScraper # TwitterScraper sınıfı burada kalmalı
...
class ToxicityClassifierApp:
    ...
    def classify_comments(self):
        ...
        if not YOUTUBE_API_AVAILABLE:
             ...
             return
        if not PREPROCESSING_AVAILABLE: # Zaten PREPROCESSING_AVAILABLE True ise fonksiyonların hepsi yüklü demektir
            messagebox.showwarning("Uyarı", "Metin ön işleme kullanılamıyor. Cümle sınıflandırılamıyor.")
            return
        ...
        # fetch_comments_from_youtube bir coroutine döndürdüğü için,
        # onu ayrı bir thread'de asyncio.run() ile çalıştırmalıyız. 
        # Bu fonksiyonun kendisi async OLMALI.
        async def fetch_and_process_comments():
            comments = await fetch_comments_from_youtube(
                youtube_url, comment_size, self.API_KEY, YOUTUBE_API_AVAILABLE, build, clean_text # 'clean_html_tags_and_time' yerine 'clean_text'
            )
        ...
        # Async fonksiyonu ayrı bir thread'de çalıştır
