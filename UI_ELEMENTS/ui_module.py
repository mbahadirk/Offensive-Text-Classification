...
class AppUI:
    ...
    def _create_widgets(self, twitter_scraper_available):
        ...
        tab_parent.pack(side="left", fill="y", padx=(0, 10), pady=5)

        # YouTube Tab
        self.youtube_tab = ttk.Frame(tab_parent)
        tab_parent.add(self.youtube_tab, text="YouTube")
        ttk.Label(self.youtube_tab, text="Video URL girin:", style="White.TLabel").pack(anchor="w", padx=5, pady=5)
        ...
        self.yt_controls_frame.pack(pady=5, fill="x", padx=5)
        ...
        self.btn_classify_comments.pack(side="left", padx=2)
        self.btn_list_toxic_comments = ttk.Button(self.yt_btn_frame, text="Sadece Toksikler")
        self.btn_list_toxic_comments.pack(side="left", padx=2)
        self.btn_list_non_toxic_comments = ttk.Button(self.yt_btn_frame, text="Sadece Non-Toksikler")
        self.btn_list_non_toxic_comments.pack(side="left", padx=2)

        # Twitter Tab
        self.twitter_tab = ttk.Frame(tab_parent)
        tab_parent.add(self.twitter_tab, text="X")
        ttk.Label(self.twitter_tab, text="Tweet URL girin:", style="White.TLabel").pack(anchor="w", padx=5, pady=5)
        ...
        self.btn_classify_manual.pack(pady=10)
        ...
        ttk.Label(right_panel, text="Model Dosyası Seç:", font=("Segoe UI", 11, "bold"), style="White.TLabel").pack(anchor="w", padx=5, pady=(10, 0))
        self.model_combo = ttk.Combobox(right_panel, state="readonly")
        ...
        self.btn_export_results = ttk.Button(right_panel, text="Sonuçları Dışa Aktar")
        self.btn_export_results.pack(pady=10, padx=5, fill="x")
        ...
        frame2 = tk.Frame(self.root, height=80, bg="#ffffff")
        frame2.pack(fill="x", padx=10, pady=10)
        ttk.Label(frame2, text="Toksiklik Eşiği (%):", font=("Segoe UI", 11, "bold"), style="White.TLabel").pack(anchor="w", padx=5, pady=5)
        self.threshold_slider = ttk.Scale(frame2, from_=0, to=100, variable=self.threshold_var, orient="horizontal", length=400)
        self.threshold_slider.pack(padx=5, fill="x", expand=True)

        self.threshold_value_label = ttk.Label(frame2, text=f"{self.threshold_var.get():.1f}%", style="White.TLabel")
        self.threshold_value_label.pack(anchor="w", padx=5)
        self.threshold_var.trace("w", lambda *args: self.threshold_value_label.config(text=f"{self.threshold_var.get():.1f}%"))

        # Frame 3: Analysis Results
        frame3 = tk.Frame(self.root, bg="#f9f9f9")
        frame3.pack(fill="both", expand=True, padx=10, pady=10)
        ttk.Label(frame3, text="Analiz Sonuçları:", font=("Segoe UI", 11, "bold"), style="TLabel").pack(anchor="w")
        self.result_text = ScrolledText(frame3, wrap="word", height=15, font=("Segoe UI", 9))
